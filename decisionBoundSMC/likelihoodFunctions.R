...
# Functions to compute likelihood for single and multiple trials. These functions are the engines for parameter estimation through MCMC (using BayesianTools) and particle filtering.
# The data come from an expanded judgment paradigm so that for each trial we have a sequence of evidence samples, 'wait' decisions and a final 'go' decision. 
# The model assumes a decision threshold with a certain height (alpha) and gradient (beta) and perfect integration of evidence. At each time-point, the agent generates a probabilistic wait/go decision depending on how close the evidence is to the decision threshold. The extent to which the agent behaves in a probabilistic or deterministic manner is controlled by a noise parameter (eta). So there are three parameters (alpha, beta and eta).
...
logLikOverall <- function(params, data){
  ...
  for(i in 1:nTrials){
    trialData <- filter(data, trialID == trialIDs[i]) # select evidence and action sequences for the current trial
