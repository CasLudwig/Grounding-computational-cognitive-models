...
# Function that runs the simulations of an agent with only coarse, categorical knowledge of the objective, for different arguments (e.g. different noise levels and window sizes). Because the agent with categorical knowledge satisfices (stops as soon as the objective estimate is "very good"), we compare its performance with a "standard" satisficing agent with a similar minimum threshold for stopping. 

categoricalObjectiveSimulation <- function(simulationN = NULL, 
                                  totalN = NULL, 
                                  sigma0 = NULL,
                                  windowSize = NULL,
                                  objRepresentation = "categorical"){
  ...
  rho <- 0.75
  peak <- 100
  satisficeThreshold <- 80 # Only need this for the agent with metric knowledge
  ...
  # If requested, visualise the objective.
  plotObj <- ggplot() +
  ...
  # Now run a satisficing agent with metric knowledge of the underlying objective scale. We vary the window size and noise in the objective estimates
  ## Start by setting up a dataframe that holds the simulation parameters and results.
  simulationRun <- seq(1,simulationN)
  simulationResults <- expand.grid(windowSize, sigma0, objRepresentation, totalN,  simulationRun)
  colnames(simulationResults) <- c("windowSize", "sd0", "objRepresentation", "N", "run")
  ...
  for(i in 1:nrow(simulationResults)){
    ...
    if(simulationResults$objRepresentation[i] == "metric"){
      ...
      tmpResult <- LoSaCo2(sd0 = simulationResults$sd0[i], 
                           stepParms = c(simulationResults$stepSD1[i],
                                         simulationResults$stepSD2[i]),
                           nMaxSteps = simulationResults$maxSteps[i],
                           xrange = c(min(theta1), max(theta1)),
  ...
  simulationSummary <- simulationResults %>%
    group_by(., sd0, windowSize, objRepresentation, N) %>%
  ...
  plotGain <- ggplot(data = simulationSummary, aes(x = windowSize, y = meanTotalObjective, colour = objRepresentation)) +
    ...
    scale_color_manual(values = cbbPalette[1:length(objRepresentation)]) +
    labs(x = TeX("window size"), y = TeX("gain per trial"), color = "objective estimates") +
    theme_bw() +
    theme(aspect.ratio = 1) +
  ...
  plotLocs <- ggplot(data = simulationSummary, aes(x = windowSize, y = meanUniqueLocs, colour = objRepresentation)) +
    geom_point(size = 4, pch = 16, position=position_dodge(width=0.5)) +
    geom_line(size = 2) +
    ...
    scale_color_manual(values = cbbPalette[1:length(objRepresentation)]) +
    labs(x = TeX("window size"), y = TeX("# unique locations"), color = "objective estimates") +
    theme_bw() +
    theme(aspect.ratio = 1) +
    theme(panel.grid.minor = element_blank()) +
    theme(strip.text = element_text(size = 14, face = "bold")) +
  ...
  plotSpread <- ggplot(data = simulationSummary, aes(x = windowSize, y = meanSpread, colour = objRepresentation)) +
    geom_point(size = 4, pch = 16, position=position_dodge(width=0.5)) +
    geom_line(size = 2) +
    ...
    scale_color_manual(values = cbbPalette[1:length(objRepresentation)]) +
    labs(x = TeX("window size"), y = TeX("spatial spread"), color = "objective estimates") +
    theme_bw() +
    theme(aspect.ratio = 1) +
    theme(panel.grid.minor = element_blank()) +
    theme(strip.text = element_text(size = 14, face = "bold")) +
