...
# March 2024: 
#   - Ensure some hard constraints so that the agent does not stray outside a permissible range.

LoSaCo1 <- function(sd0 = 1, stepParms = c(1,1), nMaxSteps = NULL, xrange = NULL, yrange = NULL, N = NULL, W = NULL, satisfice = 1, muLoc = c(0,0), width = c(1,1), rho = 0, peak = 1){
  ...
  satisfice = satisfice * peak # put relative stopping criterion (0 - 1) on the actual objective scale
  ...
  for(stepCount in 2:nMaxSteps){
    ...
    if(takeStep == 0){ # Stay at the current location
      ...
      # Generate candidate steps until the step falls within the permissible range.
      permissibleStep <- FALSE
      while(!permissibleStep){
        xyNew <- rnorm(2,mean = c(xyGrid$theta1[stepCount], xyGrid$theta2[stepCount]), sd = stepParms)
        if(between(xyNew[1], xrange[1], xrange[2]) && between(xyNew[2], yrange[1], yrange[2])){
          permissibleStep <- TRUE
        }
      }
