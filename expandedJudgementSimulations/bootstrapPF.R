...
summaryStats <- function(pf){
  ...
  pfSummary <- data.frame(trial = rep(1:nTrials, each = nVars),
...
pfRun <- function(data, n = 2000, hyperParms = NULL){
  ...
  for(i in 1:nTrials){
    ...
    currTrial <- filter(data, trial == trials[i]) # Select data for the current trial (evidence and action sequence)
